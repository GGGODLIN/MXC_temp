...
const Utils = {
  ...
  // 获取最小公倍数
  getCommonMultiple(num) {
    let arr = [];
    let i = 0;
    while (true) {
      let commonMultiple = i * num;
      i++;
      if (commonMultiple >= 10) {
        break;
      } else {
        arr.push(commonMultiple);
      }
    }
    return arr;
  },
  // 价格最小精度差
  stepNum(num, arr, accuracy, unit) {
    num = String(num);
    if (num.indexOf('.') > 0) {
      let numArr = num.split('.');
      if (numArr.length == 2 && numArr[1].length > 0 && numArr[1].length < accuracy) {
        return num;
      }
      if (numArr.length == 2 && numArr[1].length > 0 && numArr[1].length > accuracy && unit) {
        let targetNum = Number(numArr[1].charAt(accuracy - 1));
        let decimalArr = numArr[1].split('');
        for (let i = 0; i < arr.length - 1; i++) {
          if (targetNum >= arr[arr.length - 1]) {
            decimalArr[accuracy - 1] = arr[arr.length - 1];
            break;
          }
          if (arr[i] < targetNum && targetNum < arr[i + 1]) {
            decimalArr[accuracy - 1] = arr[i];
            break;
          }
        }
        decimalArr.length = accuracy;
        return String(this.precision.plus(Number(numArr[0] + '.' + decimalArr.join('')), Number(unit)));
      }
      if (numArr.length == 2 && numArr[1].length > 0 && numArr[1].length === accuracy) {
        let lastNum = numArr[1].charAt(numArr[1].length - 1);
        if (arr.some(v => v == lastNum)) {
          return num;
        }
        if (lastNum > arr[arr.length - 1]) {
          if (numArr[1].length == 1) {
            return String(Number(numArr[0]) + 1);
          } else {
            let decimalArr = numArr[1].split('');
            let newDecimalArr = [];
            decimalArr.forEach((v, index) => {
              if (index < decimalArr.length - 1) {
                newDecimalArr.push(v);
              }
            });
            let lastIndex = newDecimalArr.length - 1;
            newDecimalArr[lastIndex] = Number(newDecimalArr[lastIndex]) + 1;
            return numArr[0] + '.' + newDecimalArr.join('');
          }
        }
        let decimalArr = numArr[1].split('');
        arr.reduce((total, curren) => {
          if (lastNum > total && lastNum < curren) {
            decimalArr[decimalArr.length - 1] = curren;
          }
          return curren;
        });
        return numArr[0] + '.' + decimalArr.join('');
      }
    }
    return num;
  },
  ...
  parseQueries() {
    let s = decodeURIComponent(window.location.search.replace('?', ''));
    let pairs = s.split('&');
    let query = {};
    pairs.forEach(p => {
      let pair = p.split('=');
