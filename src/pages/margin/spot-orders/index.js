...
function Orders({ location }) {
  ...
  const filterSubmit = () => {
    ...
    params.accountType = 'STEP';
    if (filterOrderType) {
      params.tradeType = filterOrderType === 1 ? 'BUY' : 'SELL';
    }

    if (filterOrderStatus) {
      params.status = filterOrderStatus;
    }

    if (filterOrderCurrency && filterOrderMarket) {
      params.symbol = `${filterOrderCurrency}_${filterOrderMarket}`;
    }
  ...
  const switchTab = useCallback(
    index => {
      ...
      if (index === 0) {
        ...
        setCurrentFilterParams({ ...currentFilterParams });
      }
      ...
      if (index === 1) {
        ...
        setAllFilterParams({ ...allFilterParams });
      }
    },
          ...
          <div>
            ...
            <div className={styles['filter-section']}>
              ...
              <Flex justify="between" wrap="wrap">
                <span
                  onClick={() => setFilterOrderStatus('NEW')}
                  className={classNames(styles['filter-btn'], { [styles.current]: filterOrderStatus === 'NEW' })}
                >
                ...
                <span
                  onClick={() => setFilterOrderStatus('FILLED')}
                  className={classNames(styles['filter-btn'], { [styles.current]: filterOrderStatus === 'FILLED' })}
                >
                ...
                <span
                  onClick={() => setFilterOrderStatus('PARTIALLY_FILLED,PARTIALLY_CANCELED')}
                  className={classNames(styles['filter-btn'], {
                    [styles.current]: filterOrderStatus === 'PARTIALLY_FILLED,PARTIALLY_CANCELED'
                  })}
                >
                ...
                <span
                  onClick={() => setFilterOrderStatus('CANCELED')}
                  className={classNames(styles['filter-btn'], { [styles.current]: filterOrderStatus === 'CANCELED' })}
                >
