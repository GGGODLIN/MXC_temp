...
import { Modal, InputItem, Tabs, Toast } from 'antd-mobile';
...
const getFavorites = memoize(markets => {
  let favorites = [];
  markets.forEach(m => {
    favorites = favorites.concat(m.list.filter(i => i.margin && i.favorite));
  });
  return favorites;
});
...
const Pairs = ({ dispatch, user, currentPair, firstLoadedMarkets, markets, visible, onClose, lastPair }) => {
  ...
  useEffect(() => {
    ...
    if (!currentPair.margin) {
      changePair(['BTC', 'USDT']);
    }
  }, [user.token, symbol, currentPair.type, firstLoadedMarkets]);

  const marginMarkets = cloneDeep(markets).map(item => {
    item.list = item.list.filter(i => i.margin);
    return item;
  });
  const renderMarkets = getRenderMarkets(marginMarkets);
  ...
  if (activeMarket === 'market.new' || activeMarket === 'market.assess') {
    const limits = markets[markets.length - 1];
    if (limits) {
      limitList = Object.entries(
        groupBy(
          limits.list.filter(item => item.margin),
          item => item.market
        )
      ).map(([marketName, list]) => ({
        marketName,
  ...
  const marketTabs = renderMarkets
