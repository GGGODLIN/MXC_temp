...
import {
  newCancelMarginOrder,
  newCancelMarginTriggerOrder,
  newMakeMarginLimitOrder,
  newmarginAvlBorrow,
  newMarginTriggerOrder,
  deleteFavorite,
  addFavorite
} from '@/services/api';
...
import Tip from '@/pages/trade/spot/Tip';
...
const Spot = ({
  ...
  const [marginModeModalVisible, setMarginModeModalVisible] = useState(false);
  const [avlBorrow, setAvlBorrow] = useState({
    currency: {},
    market: {}
  });
  const [isShowOpenAccount, setIsShowOpenAccount] = useState(false);
  const isAuto = accounts.find(i => i.symbol === symbol) && accounts.find(i => i.symbol === symbol).tradeMode === 'AUTO';
  ...
  useEffect(() => {
    ...
    if (user.token && isOpenMarginV2 && currency && market && connectState === 'ok') {
      dispatch({
  ...
  }, [user.token, symbol, isOpenMarginV2, orders.length]);

  const [isPresale, setIsPresale] = useState(false);

  const getAvlBorrow = () => {
    newmarginAvlBorrow(symbol).then(res => {
      if (res.code === 200) {
        const borrow = {};
        borrow.currency = res.data.find(i => i.currency === currentPair.currency);
        borrow.market = res.data.find(i => i.currency === currentPair.market);
        setAvlBorrow(borrow);
      }
    });
  };
  ...
  if (user.token && orders.length) {
    for (let i = 0; i < orders.length; i++) {
      ...
      if (orderListMode === 1) {
        o.percent = (100 * o.execQty) / o.orderQty;
        o.dealAmount = o.execAmt;
        o.dealAmountString = cutFloatDecimal(o.dealAmount, currentPair.priceScale);
        const p = o.dealAmount / o.execQty;
        o.avgPrice = p || 0;
  ...
  const onSliderChange = val => {
    ...
    let mb = marketBalance.available || 0;
    let cb = currencyBalance.available || 0;
    if (isAuto) {
      mb = (marketBalance.available || 0) * 1 + avlBorrow.market.avlBorrow * 1;
      cb = (currencyBalance.available || 0) * 1 + avlBorrow.currency.avlBorrow * 1;
    }
  ...
  const cancelOrder = id => {
    newCancelMarginOrder(id).then(res => {
  ...
  const cancelOpenTriggerOrder = id => {
    ...
    newCancelMarginTriggerOrder(id).then(res => {
      if (res.code === 0 || res.code === 200) {
        Toast.success(formatMessage({ id: 'trade.entrust.cancel_success' }));
      }
    });
  ...
  const doPlaceTriggerOrder = params => {
    ...
    newMarginTriggerOrder(params).then(res => {
      if (res.code === 0 || res.code === 200) {
  ...
  const doPlaceLimitOrder = params => {
    setCreateOrderLoading(true);
    newMakeMarginLimitOrder(params).then(res => {
      if (res.code === 0 || res.code === 200) {
        Toast.success(formatMessage({ id: 'trade.entrust.place_success' }));
        dispatchInput({
          quanInput: 0,
  ...
  const MarginSafeReteTip = () => {
    Modal.alert(null, formatMessage({ id: 'margin.title.safey_ratio_text' }));
  };

  const openMarginAccount = () => {
    if (!user.id) {
      gotoLogin();
    } else if (isOpenMarginV2) {
      router.push(`/uassets/margin-detail?coin=${currentPair.currency}_${currentPair.market}`);
    } else {
      setIsShowOpenAccount(!isShowOpenAccount);
    }
  };

  const toggleOrderList = t => {
    setOrderListMode(t);
    const params = {
      accountType: 'STEP',
      pageNum: 1,
    ...
    if (t === 2) {
      dispatch({
        type: 'margin/getMarginTriggerOrder',
        payload: params
      });
    }
  };
  ...
  const currentMargin = margins.find(i => i.symbol === `${currentPair.currency}/${currentPair.market}`) || {};
  return (
    <>
      <div className={styles.header}>
        <div className={styles.titleWrapper}>
          <div className={styles.title}>
            <div onClick={() => router.push('/trade/spot')}>{formatMessage({ id: 'header.trade_center' })}</div>
            <div className={styles.active}>{formatMessage({ id: 'margin.title.margin' })}</div>
        ...
        <div className={styles.pairWrapper}>
          <div className={styles.pairInner}>
            <span onClick={() => setPairsModalVisible(true)}>
              ...
              <b className={styles.marginRatio}>X{currentMargin.ratio}</b>
            </span>
      ...
      <div className={styles.marginAccount} onClick={() => setMarginModeModalVisible(true)}>
        <div>
          <p>
            {formatMessage({ id: 'margin.title.safey_ratio' })}{' '}
            <b>
              {' '}
              {Number(account.riskRate) === 0 || Number(account.riskRate) >= 2 || isNaN(Number(account.riskRate)) ? (
                '≥200%'
              ) : (
                <>={cutFloatDecimal(account.riskRate * 100, 0)}%</>
              )}
            </b>{' '}
            <i className="iconfont icontishi" onClick={MarginSafeReteTip}></i>
          </p>
          <p>
            {formatMessage({ id: 'margin.title.break_price' })}{' '}
            <b>
              {!isNaN(Number(account.liquidationPrice)) ? cutFloatDecimal(account.liquidationPrice, currentPair.priceScale) : '--'}{' '}
              {currentPair.market}
            </b>
          </p>
        </div>
        <div onClick={openMarginAccount}>
          <b className={styles.enter}>
            {formatMessage({ id: 'margin.title.toLoan' })}/{formatMessage({ id: 'margin.title.toBack' })}
          </b>
          ...
          <div className="m-t-15">
            {direction === 'bid' ? (
              <>
                ...
                {isAuto && (
                  <div className={styles.balanceItem}>
                    <span>{formatMessage({ id: 'margin.title.auto_amount' })}</span>
                    <span>{`${cutFloatDecimal(avlBorrow.market.avlBorrow, currentPair.priceScale)} ${currentPair.market}`}</span>
                  </div>
                )}
                <div className={styles.balanceItem}>
              ...
              <>
                ...
                {isAuto && (
                  <div className={styles.balanceItem}>
                    <span>{formatMessage({ id: 'margin.title.auto_amount' })}</span>
                    <span>{`${cutFloatDecimal(avlBorrow.currency.avlBorrow, currentPair.priceScale)} ${currentPair.currency}`}</span>
                  </div>
                )}
                <div className={styles.balanceItem}>
                  <span>{formatMessage({ id: 'trade.spot.action.frozen' })}</span>
        ...
        <div className={styles.orderHeader}>
          <div className={styles.orderNav}>
            ...
            <div
              ...
              onClick={() => router.push({ pathname: '/margin/spot-orders', hash: `#${currentPair.currency}_${currentPair.market}` })}
            >
          ...
          <div className={styles.orderTabs}>
            <div className={cs(styles.orderTab, 'm-r-20', orderListMode === 1 && styles.orderTabActive)} onClick={() => toggleOrderList(1)}>
              {formatMessage({ id: 'trade.box.limit_trade' })}
            </div>
            <div className={cs(styles.orderTab, orderListMode === 2 && styles.orderTabActive)} onClick={() => toggleOrderList(2)}>
      ...
      {/* <Modal popup animationType="slide-up" visible={marginModeModalVisible} onClose={() => setMarginModeModalVisible(false)}>
        <div className={styles.marginAccountInfo}>
          <h3>{formatMessage({ id: 'margin.title.account' })}</h3>
          <p className={styles.accountTip}>
            当风险率≤110%时，账户将触发爆仓以归还借币和利息。
            <br /> 风险率=杠杆账户资产/(借币+利息)*100%
          </p>
          <div className={styles.accountInfo}>
            <span>{formatMessage({ id: 'margin.title.safey_ratio' })}</span>
            <b>
              {formatMessage({ id: safetyText[safety.safeRatioStatus] })}{' '}
              {safety.safeRatio > 2 ? '≥ 200%' : `${cutFloatDecimal(safety.safeRatio * 100, 0)}%`}
            </b>
          </div>
          <div className={styles.accountInfo}>
            ...
            <b>
              {lang.startsWith('zh') ? (
                <>{` ≈ ${cutFloatDecimal(safety.totalAssetQty * cnyPrices.USDT, 2) || '--'} CNY`}</>
              ) : (
                  <>{` ≈ ${safety.totalAssetQty || '--'} USD`}</>
                )}
            </b>
          </div>
          <div className={styles.accountInfo}>
            <span>{formatMessage({ id: 'margin.title.my_loan' })}</span>
            <b>
              {safety.marginQty || '--'} {safety.marginCrncy}
            </b>
          </div>
          <div className={styles.accountBtns}>
            <Button onClick={() => router.push('/uassets/transfer?type=margin&currency=USDT')} inline size="small" type="ghost">
              {formatMessage({ id: 'assets.transfer' })}
            </Button>
            <Button onClick={() => router.push('/margin/loan')} inline size="small" type="ghost">
              {formatMessage({ id: 'margin.title.toLoan' })}
            </Button>
            <Button onClick={() => router.push('/margin/back')} inline size="small" type="ghost">
              {formatMessage({ id: 'margin.title.toBack' })}
            </Button>
          </div>
        </div>
      </Modal> */}
      {marginChecking && <RegisterModal isRegist={isShowOpenAccount} />}
      <Pairs visible={pairsModalVisible} onClose={() => setPairsModalVisible(false)} />
    </>
  );
};

export default connect(({ trading, auth, margin, wsStatus, setting }) => ({
  user: auth.user,
  ...
  triggerOrders: margin.triggerOrders,
  asks: trading.asks,
  bids: trading.bids,
  ...
  currentPairBalance: margin.currentPairBalance,
