...
import { ListView, Picker, PullToRefresh, WingBlank } from 'antd-mobile';
...
function Container() {
      ...
      <section className={styles.item}>
        <div className={styles['value-wrapper']}>
          ...
          <div className={styles['text-right']}>
            ...
            <p>
              {item.forkType === 'CONVERT'
                ? formatMessage({ id: 'fork.conversion.btn.submit' })
        ...
        <div className={styles['value-wrapper']}>
          <div>
            <p>{formatMessage({ id: 'fork.conversion.record.num1' })}</p>
            <p>{item.forkType === 'CONVERT' ? numberToString(-item.amount) : numberToString(item.amount)}</p>
          </div>
          <div>
            <p>{formatMessage({ id: 'fork.conversion.record.num2' })}</p>
            <p>{item.forkType === 'CONVERT' ? numberToString(item.amount) : numberToString(-item.amount)}</p>
          </div>
          <div className={styles['text-right']}>
            <p>{formatMessage({ id: 'fork.conversion.record.num3' })}</p>
            <p>{item.forkType === 'CONVERT' ? numberToString(item.amount) : numberToString(-item.amount)}</p>
          </div>
        </div>
      </section>
  ...
  return (
    <ThemeOnly theme="light">
      <TopBar>{formatMessage({ id: 'fork.conversion.record.link' })}</TopBar>

      <div className={styles.wrapper}>
        {!fromBrowser && <h3 className={styles.title}>{formatMessage({ id: 'fork.conversion.record.link' })}</h3>}
