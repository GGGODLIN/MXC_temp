...
const Deif = ({ webEtfCurrency, webCurrency, markets }) => {
  ...
  const getPartition = async () => {
    ...
    if (res.code === 200) {
      const bscData = res.data.find(item => item.entryKey === 'mc-trade-zone-web3');
      setZones(bscData);
    }
  };
  ...
  return (
    <div className={styles.ecologyContent}>
      <TopBar>{formatMessage({ id: 'mc_ecology_web3_title' })}</TopBar>
      <div className={styles.headerBg}>
        <div className={styles.title}>{formatMessage({ id: 'mc_ecology_web3_title' })}</div>
      </div>
...
export default connect(({ auth, trading, etfIndex, defi }) => ({
  ...
  webCurrency: defi.webCurrency,
  webEtfCurrency: defi.webEtfCurrency
}))(Deif);
