...
const Deif = ({ markets }) => {
  ...
  const getPartition = async () => {
    ...
    if (res.code === 200) {
      const bscData = res.data.find(item => item.entryKey === 'mc-trade-zone-defi');
      setZones(bscData);
    }
  };
  const getInfo = async () => {
    let params = {
      plateKey: 'mc-trade-zone-defi'
    };
  ...
  return (
    <div className={styles.ecologyContent}>
      <TopBar>{formatMessage({ id: 'ecology.defi.title' })}</TopBar>
      <div className={styles.headerBg}>
        <div className={styles.title}> {formatMessage({ id: 'ecology.defi.title' })}</div>
      </div>
