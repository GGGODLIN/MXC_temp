...
const Deif = ({ markets }) => {
  ...
  const getPartition = async () => {
    ...
    if (res.code === 200) {
      const bscData = res.data.find(item => item.entryKey === 'mc-trade-zone-metaverse');
      setZones(bscData);
    }
  };
  const getInfo = async () => {
    let params = {
      plateKey: 'mc-trade-zone-metaverse'
    };
  ...
  return (
    <div className={styles.ecologyContent}>
      <TopBar>{formatMessage({ id: 'mc_defi_metaversezone_title' })}</TopBar>
      <div className={styles.headerBg}>
        <div className={styles.title}>{formatMessage({ id: 'mc_defi_metaversezone_title' })}</div>
      </div>
      <div className={styles.content}>
        <div className={styles.infoContent}>
          ...
          <PopularCurrency title={formatMessage({ id: 'mc_defi_metaversezone_projects' })} type="polkaETF" list={etfList} />
        </div>
...
export default connect(({ auth, trading, etfIndex, defi }) => ({
  ...
  metaverseZoneEtfCurrency: defi.metaverseZoneEtfCurrency,
  metaverseZoneCurrency: defi.metaverseZoneCurrency
}))(Deif);
