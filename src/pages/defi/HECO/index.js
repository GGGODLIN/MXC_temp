...
const Deif = ({ markets }) => {
  ...
  const getPartition = async () => {
    ...
    if (res.code === 200) {
      const bscData = res.data.find(item => item.entryKey === 'mc-trade-zone-heco');
      setZones(bscData);
    }
  };
  const getInfo = async () => {
    let params = {
      plateKey: 'mc-trade-zone-heco'
    };
  ...
  return (
    <div className={styles.ecologyContent}>
      <TopBar>{formatMessage({ id: 'mc_defi_heco_title' })}</TopBar>
      <div className={styles.headerBg}>
        <div className={styles.title}>{formatMessage({ id: 'mc_defi_heco_title' })}</div>
      </div>
...
export default connect(({ auth, trading, etfIndex, defi }) => ({
  ...
  hecoCurrency: defi.hecoCurrency,
