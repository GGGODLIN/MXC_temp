...
const Deif = ({ markets }) => {
  ...
  const getPartition = async () => {
    ...
    if (res.code === 200) {
      const bscData = res.data.find(item => item.entryKey === 'mc-trade-zone-nft');
      setZones(bscData);
    }
  };
  const getInfo = async () => {
    let params = {
      plateKey: 'mc-trade-zone-nft'
    };
  ...
  return (
    <div className={styles.ecologyContent}>
      <TopBar>{formatMessage({ id: 'ecology.nft.title' })}</TopBar>
      <div className={styles.headerBg}>
        <div className={styles.title}> {formatMessage({ id: 'ecology.nft.title' })}</div>
      </div>
      <div className={styles.content}>
        ...
        <PopularCurrency title={formatMessage({ id: 'ecology.etfZone.hot' })} type="defi" list={zonesCoinList} />
      </div>
    </div>
  );
};
