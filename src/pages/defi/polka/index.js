...
const Deif = ({ markets, user, dispatch, theme, netValues, coinList, polkaCurrency, polkaEtfCurrency }) => {
  ...
  const getPartition = async () => {
    ...
    if (res.code === 200) {
      const bscData = res.data.find(item => item.entryKey === 'mc-trade-zone-dot');
      setZones(bscData);
    }
  };
  const getInfo = async () => {
    let params = {
      plateKey: 'mc-trade-zone-dot'
    };
  ...
  return (
    <div className={styles.ecologyContent}>
      <TopBar>{formatMessage({ id: 'ecology.polka.title' })}</TopBar>
      <div className={styles.headerBg}>
        <div className={styles.title}>{formatMessage({ id: 'ecology.polka.title' })}</div>
      </div>
      <div className={styles.content}>
        <div className={styles.infoContent}>
          ...
          <PopularCurrency title={formatMessage({ id: 'ecology.etfZone.title' })} type="polkaETF" list={etfList} />
        </div>
...
export default connect(({ auth, trading, etfIndex, defi }) => ({
  ...
  polkaEtfCurrency: defi.polkaEtfCurrency
}))(Deif);
