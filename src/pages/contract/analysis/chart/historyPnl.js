...
const HistoryPnl = ({ data, theme, time, decimal, showData }) => {
      ...
      tooltip: {
        ...
        formatter: function(param) {
          return `<div style="color: ${toolTipColor}; font-size: 12px;"><div style="background: ${toolTipTitleBg};padding-left: 4px;">${
            param[0].name
          }</div> <div>${formatMessage({ id: 'mc_contract_analysis_total_pnl' })}: <span  style="color: ${
            param[0].value > 0 ? upColor : downColor
          }">${showData ? plusPrefix(cutFloatDecimal(param[0].value, decimal)) : '****'}</span></div></div>`;
        }
      },
      grid: {
        left: '20',
