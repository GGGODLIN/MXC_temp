...
import { getFundingRateList, getFundingRate } from '@/services/contractapi';
...
function Container({ dispatch, productList, location }) {
  ...
  const getDataList = useCallback(
    (params = currentParams, type) => {
      getFundingRateList(params).then(result => {
  ...
  const pickerChangeHandle = useCallback(values => {
    ...
    props => {
      return (
        <section className={styles['custom-body']}>
          <h3 className={styles.title}>{formatMessage({ id: 'swap.informationPage.index.fundsHistory' })}</h3>

          <div className={cs(styles.explain, styles['marginBottom24'])}>
            <p>{formatMessage({ id: 'swap.informationPage.fundingList.message1' })}</p>
            <p>{formatMessage({ id: 'swap.informationPage.fundingList.message2' })}</p>
            <p>{formatMessage({ id: 'swap.informationPage.fundingList.message3' })}</p>
          </div>
    ...
    return (
      <section className={styles.item}>
        <div>
          ...
          <p className={styles.value}>{timeToString(item.settleTime)}</p>
          <p className={styles.tip}>{formatMessage({ id: 'swap.informationPage.fundingList.message4' })}</p>
          <p className={styles.value}>
            {formatMessage({ id: 'swap.informationPage.hours' }, { hours: fundingDetail && fundingDetail.collectCycle })}
          </p>
        </div>
        <div>
          <p className={styles.tip}>{formatMessage({ id: 'layout.title.tabbar.contract' })}</p>
          <p className={styles.value}>
            {currentProduct ? (language.startsWith('zh-') ? currentProduct.displayName : currentProduct.displayNameEn) : '--'}
          </p>
          <p className={styles.tip}>{formatMessage({ id: 'swap.informationPage.funding.rate' })}</p>
          <p className={styles.value}>{Utils.precision.times(item.fundingRate, 100)}%</p>
        </div>
      </section>
    );
  };
