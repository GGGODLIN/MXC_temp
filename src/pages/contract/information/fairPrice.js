...
function Container({ dispatch, productList, location }) {
  ...
  return (
    <div className={styles.content}>
      <WingBlank>
        <h3 className={styles.title}>{formatMessage({ id: 'swap.common.table.tagPrice' })}</h3>
        ...
        <div>{currentProduct && productList && <Kline lineType="fair" currentProduct={currentProduct} />}</div>

        <div className={styles.explain}>
          <p>{formatMessage({ id: 'swap.informationPage.fairPrice.tip1' })}</p>
          <p>{formatMessage({ id: 'swap.informationPage.fairPrice.tip2' })}</p>
          <p>{formatMessage({ id: 'swap.informationPage.fairPrice.tip3' })}</p>
        </div>
      </WingBlank>
    </div>
  );
