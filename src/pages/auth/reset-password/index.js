...
import { resetPasswordBySupport } from '@/services/api';
...
const ForgetPasswordReset = ({ form, location }) => {
    ...
    validateFields(async (err, values) => {
      if (!err) {
        const params = {
          ...
          resetPwdToken: location.query.token
        };
        const res = await resetPasswordBySupport(params);

        if (res.code === 0) {
          Toast.success(formatMessage({ id: 'auth.reset.password.success' }), 2, () => {
  ...
  return (
    <div>
      <TopBar>{formatMessage({ id: 'auth.reset.password' })}</TopBar>
      <WingBlank style={{ marginTop: 30 }}>
        ...
        <WhiteSpace />
        <Button type="primary" onClick={handleSubmit} style={{ marginTop: 30 }} disabled={regPasswordError || regPassword2Error}>
